...
- **SETTLED 2026-07-29 — arrival day moves compute only.** 83 keeps running `chem_beo`; the database (Atlas), the frontend bundle, nginx, TLS, DNS and Stripe are **not touched**; docking, DiffDock and Tanimoto repoint at the box **one env var at a time**, rollback = unset + restart. Replacing `chem_beo` with this repo's server + `client/dist` is a **separate release, weeks later**, with its own rehearsal. `docs/BOX-ARCHITECTURE.md` §2–§3 is the decision; `docs/ARRIVAL-RUNBOOK.md` runs **Phases 1·2·3·6·7** — **Phase 4 and Phase 5 are marked dead**. Do not plan a database migration or a frontend swap for arrival day.
