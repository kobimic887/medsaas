...
- **Prerequisite either way: lift `chem_beo`'s hardcoded service addresses into env vars.** It has no `ligandServiceConfig` and hardcodes all five Asinex URLs (`index.js:1454, 1502, 1589, 1850, 2580`) plus nine Oracle Tanimoto URLs, reading only 8 env vars, none a service address. Until that patch lands, cutover and rollback are source edits plus a restart. Also note: nothing in this repo reaches production, so the credit-refund / NVIDIA-429 / 401→502 fixes must be ported if wanted, and the credit-minting hole (`chem_beo:3343`) is not fixed by any migration.
