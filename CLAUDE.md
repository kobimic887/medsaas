...
- **🛑 The box runs `chem_beo`, not this repo** (owner, 2026-07-28). That is the legacy backend currently on 83 — see `docs/PRODUCTION-83-INVENTORY.md` §6. It removes the user-migration blocker (chem_beo matches the production data), but it also means **"cutover is config" is false**: `chem_beo` has no `ligandServiceConfig` and hardcodes all five Asinex URLs plus nine Oracle Tanimoto URLs as string literals, reading only 8 env vars, none of them a service address. Cutover and rollback are source edits plus a restart. **Recommended fix before arrival day: lift those addresses into env vars with current values as defaults.** Also: nothing in this repo ships, so the credit-refund / NVIDIA-429 / 401→502 fixes must be ported if wanted, and the credit-minting hole (`chem_beo:3343`) now survives the migration instead of being fixed by it.
- **Cutover is config, not a deploy — TRUE ONLY OF THIS REPO'S SERVER, which is not what ships.** `company.ligandServiceConfig` (`server/index.js:886-906`) overrides all four Asinex URLs per company from the Company Admin UI. Kept here because it is the mechanism worth *recreating* in `chem_beo`, not because it is available today.
