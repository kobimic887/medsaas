# Requirements Archive: v2 Bun Migration
...
- [x] **RUN-03**: Startup env-var validation and all existing middleware/routes behave identically under Bun, verified by a smoke test of auth, the Stripe billing webhook, and one token-consuming simulation endpoint
...
- [x] **PKG-03**: The client Vite build still produces a working bundle when invoked through Bun (Vite retained; bundler swap is out of scope)
...
- [x] **OPS-03**: The `check`, `test:brand`, and `test:stripe` scripts run under Bun
