...
| [BOX-BEFORE-AFTER.md](./BOX-BEFORE-AFTER.md) | The plain-language version: what runs where today, what runs where afterwards, where every piece of data lives on each side, what a Pyxis user notices — and the arrival-day runbook with the hardware acceptance checks. Start here if you want the picture rather than the sequence. | **planning — nothing applied** |
