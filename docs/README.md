...
| [ARRIVAL-RUNBOOK.md](./ARRIVAL-RUNBOOK.md) | **Agent-executable.** `execute the plan docs/ARRIVAL-RUNBOOK.md`. Every step with its command, expected output, and what to do on mismatch; hard rules, destructive-step gating, a resumable state file, rollback per phase, and abort conditions. **Phase 5 runs before delivery; hardware day runs 1·2·3·6·7; Phase 4 is dead.** Phase numbers are historical, not an order. | **ready to run — revised 2026-07-29** |
