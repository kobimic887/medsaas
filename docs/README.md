...
> **Start here, 2026-07-29.** The docs went through several conflicting plans. The settled one:
> **arrival day moves compute only.** 83 keeps running `chem_beo`, patched
> ([`deploy/chem_beo/`](../deploy/chem_beo/)); the database, the frontend, nginx and Stripe are
> not touched; docking, DiffDock and Tanimoto repoint at the box one environment variable at a
> time. Replacing the API server with this repo's is a **separate release, weeks later.**
> Read [BOX-ARCHITECTURE.md](./BOX-ARCHITECTURE.md) §2, then ARRIVAL-RUNBOOK's opening section.
> Anything in an older doc that says the database or the frontend moves on arrival day is stale.
...
| [scripts/migrate-legacy-users.mjs](../scripts/migrate-legacy-users.mjs) | Brings the 49 legacy user documents up to the shape this repo's server needs. Idempotent, dry-run by default. Gates the *later* release, not arrival day — but it also fixes 47 users who have never been able to run a simulation. | **written, dry-run verified — not applied** |
...
| [DOCKING-CONTRACT.md](./DOCKING-CONTRACT.md) | The `{pdb, sdf}` payload the docking engine must reproduce, captured from production while Asinex still answered. Pose format, property tags, score convention, the URL-encoded SMILES trap, and an acceptance test for the OSS rebuild. | **current** |
| [ARRIVAL-RUNBOOK.md](./ARRIVAL-RUNBOOK.md) | **Agent-executable.** `execute the plan docs/ARRIVAL-RUNBOOK.md`. Every step with its command, expected output, and what to do on mismatch; hard rules, destructive-step gating, a resumable state file, rollback per phase, and abort conditions. **Arrival day runs Phases 1·2·3·6·7 — Phase 4 and Phase 5 are marked dead at their heads.** | **ready to run — revised 2026-07-29** |
