...
| [BOX-ARCHITECTURE.md](./BOX-ARCHITECTURE.md) | **Decision record: what runs where, and why.** API stays on 83 (pick-up warranty means the box will be gone 1–3 weeks someday); box gets all compute; this repo's server replaces `chem_beo`, which has 7 of 73 routes authenticated; Atlas and CloudAMQP decisions. Supersedes the topology in every other document. | **current — read first** |
...
| [ARRIVAL-RUNBOOK.md](./ARRIVAL-RUNBOOK.md) | **Agent-executable.** `execute the plan docs/ARRIVAL-RUNBOOK.md`. Every step with its command, expected output, and what to do on mismatch; hard rules, destructive-step gating, a resumable state file, rollback per phase, and abort conditions. Carries *what to do*; the other three carry *why*. | **ready to run** |
| [COMPUTE-BOX-MIGRATION.md](./COMPUTE-BOX-MIGRATION.md) | Full trace of every machine, API and compute dependency, and the plan to move the backend onto the Amsterdam GPU box. Includes the CUDA matrix, storage layout, phased sequence, and open decisions. Read BOX-SPEC first — sections here carry inline notes where scope changed. | **planning — nothing applied** |
