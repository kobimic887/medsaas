...
| `OPEN_COMPOUNDS_AI_ENABLED` | reserved; does not activate AI in v1 |
...
**Not implemented. Configuration placeholders only.** Deterministic ChEMBL + RDKit search works without
AI. Status reports `ai.enabled: false` even when credentials are configured; an AI tool loop still needs implementation. No paid
...
Merged with staging/folding while retaining scoped stock fixes. Owner approved the tested folding UX; this does not establish live NVIDIA or durable-history evidence. Combined build, Open compounds tests, Simulation lifecycle, staging demo/simulation and auth-header checks pass. Full CI stops at four plan-UX assertions; the identical failures were reproduced on pre-merge f993ecf. No billing changes were made. Production deployment is separate; last deployed release is 7004271.
