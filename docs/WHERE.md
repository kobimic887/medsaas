...
| **Staging (owner-test)** | `84` systemd `pyxis-web-staging` → `/root/pyxis-STAGING-5274` | `https://app.pyxis-discovery.com/staging/` — nginx `location /staging/` → `127.0.0.1:5274` (loopback only). **Full app since 2026-09-24:** same production Atlas, accounts, history, credits, checkout and providers; separate frontend and loopback process. Real/virtual macrocycles use `:8274`; Open compounds AI uses private oracleOld OmniRoute tunnel. Actions can change production data. Contract + rollback: `docs/STAGING.md` / `deploy/staging/` |
...
3. Two gitless `:5174` trees: 84 live `d96a7a6…` vs 83 `b5962da`.
