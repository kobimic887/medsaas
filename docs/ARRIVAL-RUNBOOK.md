...
> (`oracleNew`) as production. Public Pyxis is systemd **`pyxis-web` `:5174`**. **`83` is
> leftover, not DNS** (owner shutdown 2026-08-21) — do **not** treat it as a long-lived
> standby/rollback host. In that state, do **not** blindly execute §8's port swap: measure
> **`84`** first. ⚠ Passages below that say “port swap on both hosts”, “mirror/`sync` to
> `83`”, or “keep `83` as verified rollback” need **owner confirmation of the post-shutdown
> rollback target** (likely on-disk / snapshot on `84` only). Do not change DNS again, and
> do not touch `oracleOld` destructively until §12's gates and a fresh explicit approval
> are complete.
>
...
| **`oracleNew`** | `84.13.81.51` | **Live production** (DNS measured 2026-08-21+; public product `:5174` since 2026-08-23). See [`POST-PROMOTION-HANDOFF.md`](./POST-PROMOTION-HANDOFF.md). Dual stack: this repo `:5174` public; legacy `:5173` / `chem_beo` `:3000` = rollback **stopped** (units still **enabled**). | After box cutover: same Amsterdam service links on this host (not leftover `83`). Not the Tanimoto source. |
...
| Which server serves the site | **already public** — `pyxis-web` `:5174` (soft flip 2026-08-23). §8 port-take of `:5173` is unused history |
...
2. **On leftover `83.229.87.94`: do not modify nginx, TLS, DNS, the firewall, or any other
   application.** It is **not DNS** and is **not** production. Inventory read/copy only. There
   is **no** port-swap exception — §8 is unused history. Do not mutate toward shutdown.
...
9. **Do not `pkill -f "bun index.js"` on `84`.** It matches production's own process. `83` is
   leftover, not production. Kill rigs by measured PID only.
...
| `<83_HOST>` / `<83_USER>` | leftover host `83.229.87.94` — inventory only, **not** needed for public flip | historical §8 |
...
| `.env` values | for the Amsterdam box's services (`deploy/box/.env.example`) **and the live app host `84`**: the Pyxis API env must be present on `oracleNew` (`/root/pyxis-LIVE-5174/server/.env`). `83` is leftover. Do not assume a source-tree copy includes secrets | §5, §9 |
...
| **`.env` files** — root, `server/`, and `deploy/box/` | secrets | `.env.example` and `deploy/box/.env.example` are templates. The live Pyxis API env must be present on `/root/pyxis-LIVE-5174/server/.env` on `oracleNew`. `83` leftover paths are inventory only. Copy/supply securely and compare only redacted fingerprints. Never overwrite a production env from a template |
...
**Then the one test that decides the architecture** — run it *from the live app host (`84`)*, not from the box. Dated text that said “from 83” is leftover-host wording.

```bash
# on the box:  sudo python3 -m http.server 443
# from 84 (live app host; historical copies said 83):
...
certificate, host firewall admitting only the live app host (`84`). Dated “only 83” below is leftover-host wording. Nothing else changes.
...
works even when inbound does not.** Invert the direction: the **box dials out to the live app
host (`84`)** and holds a persistent tunnel; `84` then talks to a local port as if the
services were on localhost. Dated “to 83” recipes below are leftover-host wording.
...
Every image pins `--platform linux/amd64`, and for a long time no host could run them: leftover `83` is 2 cores / 1 GB (an OOM there is not the live site), and `oracleOld`/`oracleNew` are arm64 with no `binfmt` emulation. **Owner's call, unchanged: build them on the Amsterdam box.** It is the right
...
currently carries production (`84`) or leftover services. `oracleOld` was upgraded to **4 cores /
...
## 6. Ingress — public hostname over HTTPS, firewalled to the live app host

> **Current (2026-08-23):** the live app host is **`84`**, not leftover `83`. Allowlist /
> probe / tunnel targets below that still say “83” mean **the live application host** —
> `84.13.81.51`. Keep the dated 83-IP recipe as history; do not allowlist leftover `83`
> as if it were still DNS.
>
...
> or the box is behind NAT, **§1c Branch B** (a box-initiated reverse tunnel to the live app
> host) is the fallback, and the rest of this section does not apply.
...
3. The host firewall admits **only the live app host (`84`) IP** to `:443`. Dated
   “only 83's IP” recipes mean that same allowlist role. That is the allowlist, and it is one
...
| `84` loopback `127.0.0.1:8001/convertSTR` (interim; leftover `83:8001` is dead) | `POST /convertSTR` on `:8001` | `handle /convertSTR*` |
...
### Then retire the legacy stack in stages — leftover `83` has 1 GB of RAM

> **Current (2026-08-23):** on live `84`, rollback units are already **stopped** (still
> enabled). Do not re-start them for box day. The RAM argument below is the 2026-08-01
> `83` record.

**Owner's call, 2026-08-01: the swap stays a swap, but the standby does not stay running
forever if it costs performance.** That caveat mattered more than it sounded, because leftover
`83` is **2 cores and 1 GB of RAM**, shared with an unrelated project on `:4000` and a
...
|---|---|---|---|
| 1 | `dockingApiUrl` | `https://<BOX_DOMAIN>/docking` | PATCH the shared Atlas company document **once**, then verify from live `84` |
...
host-local `.env` values on **`84`** and restart `pyxis-web`. Keep those URLs valid — they
are the disaster-recovery path, and the box has a 1–3 week repair time.
Before §12 cleanup, also update the legacy `chem_beo` Tanimoto target on the `84` rollback
tree (or explicitly document that legacy rollback no longer includes Tanimoto); otherwise a
rollback to the legacy frontend after deleting `oracleOld` silently loses Deep Similarity.
...
After the Amsterdam copy is restored, verify the Tanimoto API from live `84`
through the real Pyxis path—not only with a direct health request. Only then change the
legacy rollback path on `84` (start the stopped rollback units if you must test them):
replace `chem_beo`'s hardcoded `http://151.145.91.17:8000` target with the verified Amsterdam
Tanimoto endpoint, test the legacy `/tanimoto/*` routes, and confirm no production path still
reaches `oracleOld`. Do not delete `oracleOld` first and hope the legacy rollback will find
the new endpoint.
...
answers the same queries correctly and `/tanimoto/*` has been repointed on live `84`, then
checked from a browser. The old Oracle containers are the rollback source until that
...
1. **convertSTR** — live interim is already `84` loopback `pyxis-convertstr`
   (`http://127.0.0.1:8001/convertSTR`). Repoint `SDF_CONVERTER_URL` to
   `https://<BOX_DOMAIN>/convertSTR` when box ingress exists. Leftover `83:8001` is not
   the public path (it has been down).
2. **RabbitMQ, then the ADMET worker.** ⚠ **Prerequisite: the box's IP must be on the Atlas
   ...
   **FinSrv is separate.** Its `MONGO_URI` belongs to a different Atlas project/cluster than
   ...
   validate FinSrv with an authenticated database-backed request on live `84`
   before calling the host healthy. Leftover `83` is not a FinSrv standby.
...
> Tanimoto is live on Amsterdam and live `84` is verified. It requires its own
...
- [ ] **No production path, including the `84` legacy rollback path, resolves to `151.145.91.17`**
      for Tanimoto; verify from a browser on live `84`. Leftover `83` is not DNS.
- [ ] Tanimoto queries, dataset count, similarity search, and the Deep Similarity page pass
      through the Amsterdam box from live `84`
- [ ] Live `84` is public `pyxis-web` `:5174` with Amsterdam service links. Do **not** require
      a matching port swap on leftover `83`
- [ ] The legacy `chem_beo` Tanimoto target on the `84` rollback tree is updated or the
      operator has explicitly accepted that legacy rollback loses Deep Similarity
- [ ] FinSrv has passed a real authenticated Mongo-backed check on live `84`;
