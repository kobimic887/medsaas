...
- **chem_beo (two remotes):** Both forks are the same API lineage. Active development lives in `server/`; the old local archive was removed after the migration.
- **Dashboard template:** Not a second frontend. The old local copy was removed; `client/` is the only dashboard source.
